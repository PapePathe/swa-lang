...
public:
  ...
  void Run(const std::string& _source) {
    ...
    auto proto1 = std::make_unique<ProtoExpr>("add_one", std::vector<std::string>{"val"});
    std::vector<std::unique_ptr<Expr>> body1;
    body1.push_back(std::make_unique<PrintExpr>("Inc...\n"));
    programStmts.push_back(std::make_unique<FuncExpr>(std::move(proto1), std::make_unique<BlockExpr>(std::move(body1))));

    auto mainProto = std::make_unique<ProtoExpr>("main", std::vector<std::string>{});
    std::vector<std::unique_ptr<Expr>> mainBody;
    mainBody.push_back(std::make_unique<DeclarationExpr>("x", std::make_unique<NumberExpr>(21)));
    mainBody.push_back(std::make_unique<PrintExpr>("Inside Main\n"));
    
    programStmts.push_back(std::make_unique<FuncExpr>(std::move(mainProto), std::make_unique<BlockExpr>(std::move(mainBody))));
